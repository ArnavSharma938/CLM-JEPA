...
class DynamicTrimCollator:
    """Remove only batch-global left-padding columns after upstream collation.

    The upstream GSM8K dataset remains tokenized and padded to max_length=512.
    This wrapper first invokes the repository's original causal-LM collator, then
    slices a prefix iff every sequence in every available representation stream
    marks that prefix as padding. Token IDs, labels, masks, truncation, example
    order, and the maximum sequence length are otherwise unchanged.
    """

    _STREAM_SUFFIXES = ("", "_user", "_assistant")
    ...
    def __call__(self, features):
        ...
        original_width = batch["input_ids"].shape[1]
        mask_keys = [
            f"attention_mask{suffix}"
            for suffix in self._STREAM_SUFFIXES
            if f"attention_mask{suffix}" in batch
        ]
        active_columns = torch.zeros_like(batch[mask_keys[0]][0], dtype=torch.bool)
        for key in mask_keys:
            active_columns |= batch[key].to(torch.bool).any(dim=0)
        first_active = int(torch.nonzero(active_columns, as_tuple=False)[0].item())
        if first_active:
            for suffix in self._STREAM_SUFFIXES:
                for prefix in ("input_ids", "labels", "attention_mask"):
                    key = f"{prefix}{suffix}"
                    if key in batch:
                        batch[key] = batch[key][:, first_active:]
        # Qwen2 otherwise assigns positions from zero after trimming. Preserve
        # the original upstream absolute positions so this remains a pure
        # removal of masked padding computation.
        positions = torch.arange(first_active, original_width, dtype=torch.long)
        for suffix in self._STREAM_SUFFIXES:
            input_key = f"input_ids{suffix}"
            if input_key in batch:
                batch[f"position_ids{suffix}"] = positions.unsqueeze(0).expand(
                    batch[input_key].shape[0], -1
                )
        return batch
...
def setup_model_and_tokenizer(model_name, use_lora=True, lora_rank=16, pretrain=False, debug=0, seed=None):
    ...
    # Upstream passes finetune_seed here but otherwise applies it only in the
    # from-scratch branch, after pretrained LoRA/new-token initialization has
    # already occurred. Seed first so the requested fixed seed actually fixes
    # both the LoRA matrices and predictor-token embedding.
    if seed is not None:
...
class RepresentationTrainer(Trainer):
    ...
    def _last_token_index(self, input_ids, labels, attention_mask):
        if self.debug != 1:
            # Exact vectorization of the loop below: for both left- and
            # right-padded sequences, len(unpad(...)) is one beyond the final
            # active mask position. Avoid one CUDA->CPU synchronization per
            # token in the normal training path.
            positions = torch.arange(
                attention_mask.shape[1], device=attention_mask.device
            ).unsqueeze(0)
            last_active = torch.where(
                attention_mask.to(torch.bool), positions, -1
            ).amax(dim=1)
            return last_active + 1 + self.last_token
    ...
    def forward(self, model, inputs, num_items_in_batch=None):
        """
        ...
        if self.additive_mask:
            ...
            if "position_ids" in inputs:
                llm_inputs["position_ids"] = torch.cat([
                    inputs["position_ids"],
                    inputs["position_ids_user"],
                    inputs["position_ids_assistant"],
                ], dim=0)
        ...
        if num_items_in_batch is not None:
            # Transformers >=4.46 uses this total across the accumulation
            # window to make the native causal loss token-weighted and
            # invariant to gradient accumulation.
            llm_inputs["num_items_in_batch"] = num_items_in_batch
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if num_items_in_batch is not None:
            # The model has already normalized lm_loss over the entire
            # accumulation window. JEPA is a per-example mean, so each
            # microbatch contributes one equal fraction of the same window.
            jepa_loss = jepa_loss / self.current_gradient_accumulation_steps
...
def main():
    ...
    parser.add_argument("--dynamic_trim", action="store_true", help="Trim only batch-global padding columns after original upstream collation.")
    parser.add_argument("--max_steps", type=int, default=-1, help="Optional measured smoke/benchmark step limit.")
    parser.add_argument("--skip_final_save", action="store_true", help="Skip final merge/save for short execution benchmarks.")
    parser.add_argument("--no_gradient_checkpointing", action="store_true", help="Disable upstream activation checkpointing for measured A6000 profiling.")
    parser.add_argument("--dataloader_num_workers", type=int, default=0, help="Execution-only DataLoader worker setting.")
    ...
    elif args.dynamic_trim:
        data_collator = DynamicTrimCollator(tokenizer)
    else:
