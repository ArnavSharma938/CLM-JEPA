...
from jepa import add_predictor_tokens, extract_source_and_target  # noqa: E402
...
def main() -> None:
    ...
    parser.add_argument("--manifest", type=Path, default=ROOT / "data" / "gate3" / "uspto_mit_synthesis.csv")
    parser.add_argument("--dataset", default="uspto_mit_synthesis")
    parser.add_argument("--task", choices=("forward", "metabolism", "retro"))
    ...
    task = args.task or (
        "retro" if "retro" in args.dataset or "480k" in args.dataset
        else "metabolism" if "metatrans" in args.dataset
        else "forward"
    )
    batch = ReactionCollator(tokenizer, task=task)(records)
    ...
    path = args.output or ROOT / "gates" / "gate3" / "run.json"
