"""Frozen fixed-span released or paper STP diagnostics for a checkpoint."""
...
def gradient_metrics(native, auxiliary, coefficient: float) -> dict[str, float]:
    device = next(value.device for value in native if value is not None)
    native_sq = torch.zeros((), device=device)
    ...
    return {
        ...
        "stp_to_native_norm_ratio": float(
            auxiliary_norm / native_norm.clamp_min(1e-30)
        ),
        "lambda_weighted_stp_gradient_norm": float(
            coefficient * auxiliary_norm
        ),
        "lambda_weighted_stp_to_native_norm_ratio": float(
            coefficient * auxiliary_norm / native_norm.clamp_min(1e-30)
        ),
        "stp_native_gradient_cosine": float(
            dot / native_norm.clamp_min(1e-30)
...
def summarize(rows: list[dict]) -> dict:
    output = {}
    keys = (
        "native_loss", "stp_loss", "span_fraction", "native_gradient_norm",
        "stp_gradient_norm", "stp_to_native_norm_ratio",
        "lambda_weighted_stp_gradient_norm",
        "lambda_weighted_stp_to_native_norm_ratio",
        "stp_native_gradient_cosine",
    )
    for key in keys:
...
def main() -> None:
    ...
    parser.add_argument("--formulation", choices=("released", "paper"), required=True)
    ...
    method_type = (
        PaperSemanticTubePrediction
        if args.formulation == "paper" else SemanticTubePrediction
    )
    method = method_type(
    ...
    for panel_row in panel:
        ...
        output = method(model, batch, stp_weight=args.coefficient)
        ...
        span = output.sampled_spans[0]
        start, end, full = (
            (span[0], span[2], span[3]) if len(span) == 4 else span
        )
        rows.append({
            ...
            "transition_cosine": 1.0 - float(output.jepa_loss.detach()),
            "sampled_span": span,
            "span_fraction": (end - start) / full,
            **gradient_metrics(native, auxiliary, args.coefficient),
        })
        model.zero_grad(set_to_none=True)

    payload = {
        "type": "fixed_span_stp_checkpoint_diagnostic",
