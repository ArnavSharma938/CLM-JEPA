...
- High residual retrieval does not prove that the decoder can exploit the residual; the Gate 5 generative tie shows that it did not produce a measurable top-1 benefit.
...
This cLM-JEPA result is related but not the same. cLM-JEPA retained its native token loss, tied native generation at top-1, and preserves exceptionally strong pair identity in the residual. Its problem is not loss of identity information; it is that an unregularized, positive-only endpoint cosine objective can be minimized through a dominant common direction while leaving useful chemistry too small for raw cosine to weight strongly. ProteinJEPA's use of variance/covariance regularization or an EMA-style target addresses the same family of geometric shortcut, but the present evidence does not justify importing its stronger word “collapse” here.

## Exactly one recommended next experiment

Run **one seed-533 cLM-JEPA pilot that replaces the positive-only cosine JEPA term with the upstream in-batch InfoNCE JEPA loss, changing nothing else**, and compare it with the existing native seed-533 checkpoint. The acceptance criterion should be jointly prespecified: retain target variance above 10% of native, retain matched retrieval above chance, and beat native exact top-1. InfoNCE is the cleanest single-variable test because it directly requires true pairs to outrank negatives, preventing a shared mean direction from making the auxiliary loss nearly zero without a discriminative margin.
