...
### 3.4 Canonical repository splits

The author releases use a mixture of OpenNMT parallel text, Parquet, and CSV.
Those transport formats are normalized once into CSV files with explicit
`example_id`, `split`, `source`, and `target` columns. Token-separated OpenNMT
text is not a separate condition and must not be retained beside an equivalent
CSV. Dataset-specific metadata may be retained in additional columns, but it
must never be supplied to the model unless Sections 3.1–3.3 explicitly permit
it.

| Task/condition | Canonical files and exact retained definition |
|---|---|
| USPTO-MIT synthesis compatibility | `data/uspto_mit_synthesis/{train,validation,test}_r_smiles.csv`; the unchanged official paired R-SMILES splits (2,195,175 / 150,000 / 200,000 augmented rows). |
| ORDerly-forward | `data/orderly_forward/{train,test}.csv`; preserve the released train/test assignment and retain exactly one nonempty product (809,731 / 83,819 rows). Source is reactants + agents + solvents. |
| non-USPTO-forward | `data/non_uspto_forward/test.csv`; external evaluation only, using the same exactly-one-product filter (28,400 rows). |
| MetaTrans training | `data/metatrans/train.csv`; all 11,414 released parent–metabolite rows. The 251-row `released_validation.csv` is retained as an upstream reference split, not substituted for the required parent-grouped scaffold validation. |
| MetaTrans primary test | `data/metatrans/heldout_drug_test.csv`; expand the official 84-parent `testMolecules_info.csv` into 217 parent–metabolite rows while preserving parent IDs for parent-level evaluation. Do not use the repository's saved `output.csv` predictions as truth. |
| USPTO-50K retrosynthesis | `data/uspto_50k/{train,validation,test}_r_smiles.csv` in product-to-precursor direction (800,060 / 100,020 / 100,140 augmented rows), plus the author-named `train_single.csv` training release (160,012 rows). Reaction class is excluded. |
| USPTO-480K template-held-out retrosynthesis | `data/uspto_480k_template_heldout/{train,validation,test}.csv`; preserve the released local-reaction-template-held-out split (383,784 / 46,519 / 48,730 rows). The template column is audit-only metadata. |
| non-USPTO-retro | `data/non_uspto_retro/test.csv`; all 23,334 released rows, external evaluation only. |

The secondary MetaTrans scaffold validation is a derived experimental split and
must be materialized under `data/metatrans/` with its construction seed,
canonical parent/scaffold identities, and zero parent/scaffold overlap recorded
in the manifest before Gate 4 metabolism tuning. It must not overwrite the
released validation CSV.
...
Avoid a full grid. Use a small reproducible TPE-guided Hyperband/ASHA study—or an equivalent scripted successive-halving procedure if that is simpler—with epoch budgets of 1, 2, and 4 and at most 12 JEPA trials. Rank trials by the prespecified validation generative metric, with validity as a hard guardrail. Replicate the top configuration on a second seed before freezing it.
...
1. run forward prediction on ORDerly-forward;
...
- separate results on filtered ORDerly-forward and non-USPTO-forward.
...
Use `WANDB_PROJECT=clm-jepa`; keep the API key and all other credentials only in environment variables. Never write credentials to the plan, source, configuration, logs, or W&B run metadata.
