# Documentation

| Path | Purpose |
|---|---|
| [CLM_JEPA_Plan.md](CLM_JEPA_Plan.md) | Authoritative research design, gates, metrics, and stopping rules |
| [context.md](context.md) | Current state for a memoryless continuation |
| [CODE_LAYOUT.md](CODE_LAYOUT.md) | Training, evaluation, diagnostic, and hardware-specific entrypoints |
| [VJEPA2_1_CHEMFM_MAPPING.md](VJEPA2_1_CHEMFM_MAPPING.md) | Exact V-JEPA 2.1 reference lock and causal ChemFM translation |
| [reports/README.md](reports/README.md) | Executive result, tested-condition table, and consolidated report index |

Experiment reports live only under `docs/reports/`. They are organized by decision dependency: fidelity, diagnosis, MSE+SIGReg, final endpoint, mechanistic localization, SIGReg pair specificity, and the PCSF minimal-spread experiment.
