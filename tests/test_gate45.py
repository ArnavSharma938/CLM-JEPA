...
from experiments import JEPA_CANDIDATES, trial_score
from train import (
...
def test_gate4_search_is_bounded_and_covers_every_prespecified_axis():
    assert len(JEPA_CANDIDATES) == 12
    assert {row["k"] for row in JEPA_CANDIDATES} == {0, 1, 2, 3}
    assert {row["lambda_eff"] for row in JEPA_CANDIDATES} == {0.5, 1.0, 2.0, 4.0}
    assert {row["dropout"] for row in JEPA_CANDIDATES} == {0.0, 0.5, 0.75}
...
def test_gate4_trial_ranking_ignores_native_loss():
    base = {
        "validation_metrics": {"exact_top1": 0.25, "valid_rate": 0.9},
        "validation_native_loss": 2.0,
    }
    lower_loss = copy.deepcopy(base)
    lower_loss["validation_native_loss"] = 0.1
    assert trial_score(base, 0.8) == trial_score(lower_loss, 0.8)
    invalid = copy.deepcopy(base)
    invalid["validation_metrics"]["valid_rate"] = 0.7
    assert trial_score(invalid, 0.8) < trial_score(base, 0.8)
